...
const SEED_AGENTS: CreateAgentInput[] = [
  {
    ...
    actions: [
      { type: 'slack', config: { channel: '#compliance-alerts', template: '{title}' } },
  ...
  },
  {
    ...
    actions: [
      { type: 'telegram', config: { chat_id: '12345', template: '{ticker} beat!' } },
    ],
  ...
  },
  {
    ...
    actions: [
      { type: 'slack', config: { channel: '#risk-team', template: 'Risk alert: {company}' } },
    ],
  ...
  },
  {
    ...
    actions: [
      { type: 'slack', config: { channel: '#security-alerts', template: 'Vulnerability scan: {target} — risk {risk_score}/10' } },
...
export async function seedIfEmpty() {
  ...
  const mockActions = [
    ...
    { agent_id: created[4].id, agent_name: created[4].name, action_type: 'slack', target: '#security-alerts', timestamp: new Date(now - 2700000).toISOString(), detail: 'Scan complete: api.staging.example.com — CRITICAL, 3 critical vulns found' },
